...
const std::string NameDialog::s_sLabelStringNormal = "Bluetooth adapter name";
const std::string NameDialog::s_sLabelStringError = "Bluetooth adapter name\nCannot be empty!";
...
//std::cout << "NameDialog::NameDialog() content area children: " << p0ContentArea->get_children().size() << '\n';
	...
	p0ContentArea->pack_start(*m_p0VBoxName, false, false);
		...
		m_p0LabelNameDesc = Gtk::manage(new Gtk::Label(s_sLabelStringNormal));
...
int NameDialog::run(const std::string& sName)
{
	assert(!sName.empty());
	m_sOldName = sName;
	m_sName = sName;
	...
	do {
		...
		// error
		m_p0LabelNameDesc->set_text(s_sLabelStringError);
...
{
	m_p0LabelNameDesc->set_text(s_sLabelStringNormal);
}
