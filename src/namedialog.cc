...
NameDialog::NameDialog(Gtk::Window& oParent, int32_t nMaxLength) noexcept
: Gtk::Dialog()
...
int NameDialog::run(const std::string& sName, int32_t nSelectedHciId) noexcept
{
	assert(!sName.empty());
	m_sOldName = sName;
	m_sName = sName;
...
void NameDialog::onErrorOneSecond() noexcept
{
	m_p0LabelNameDesc->set_text(m_sLabelStringNormal);
}
const std::string& NameDialog::getName() const noexcept
{
	return m_sName;
}
void NameDialog::onEntryNameChanged() noexcept
{
