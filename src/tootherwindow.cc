...
, m_bSettingWidgetsValues(false)
{
	...
	const std::string sInfoTitle = "bluetoother";
	const std::string sInfoText = std::string{"\n"} +
			...
			" and the related systemd service. It also allows one to set the"
...
void TootherWindow::onWindowRealize() noexcept
{
	Glib::signal_timeout().connect_once(sigc::mem_fun(*this, &TootherWindow::showReadOnlyWarning), 0);
}
void TootherWindow::showReadOnlyWarning() noexcept
{
	assert(m_bReadOnly);
...
void TootherWindow::setWidgetsValue() noexcept
{
...
void TootherWindow::setWidgetsSensitivity() noexcept
{
	...
	if (bAdapterSelected) {
		...
		assert(oAdapter[JsonCommon::s_sKeyModelAdapterHardwareEnabled].is_boolean());
		const bool bHardwareEnabled = oAdapter[JsonCommon::s_sKeyModelAdapterHardwareEnabled];
		assert(oAdapter[JsonCommon::s_sKeyModelAdapterSoftwareEnabled].is_boolean());
		const bool bSoftwareEnabled = oAdapter[JsonCommon::s_sKeyModelAdapterSoftwareEnabled];
		//
...
void TootherWindow::startRefreshing(int32_t nMillisec) noexcept
{
...
void TootherWindow::cursorToHourglass() noexcept
{
	if (get_window()) {
...
void TootherWindow::cursorToNormal() noexcept
{
	if (m_refStdCursor) {
...
void TootherWindow::startTimeout(int32_t nStamp, int32_t nMillisec) noexcept
{
	Glib::signal_timeout().connect_once(sigc::bind(sigc::mem_fun(*this, &TootherWindow::onTimeout), nStamp, nMillisec)
										, nMillisec);
}
void TootherWindow::doReceiveString(bool bError, const std::string& sStr) noexcept
{
...
void TootherWindow::onTimeout(int32_t nStamp, int32_t nMillisec) noexcept
{
...
void TootherWindow::onNotebookSwitchPage(Gtk::Widget*, guint nPageNum) noexcept
{
...
void TootherWindow::execAdapterBoolCmd(const std::string& sCmd, bool bEnabled, int32_t nTimeout) noexcept
{
	cursorToHourglass();
...
void TootherWindow::execAdapterStringCmd(const std::string& sCmd, const std::string& sStr, int32_t nTimeout) noexcept
{
	cursorToHourglass();
...
void TootherWindow::onSoftwareEnabledToggled() noexcept
{
	if (m_bSettingWidgetsValues) {
		return;
	}
...
void TootherWindow::onAdapterIsUpToggled() noexcept
{
	if (m_bSettingWidgetsValues) {
		return;
	}
...
void TootherWindow::onButtonSetName() noexcept
{
	if (m_bSettingWidgetsValues) {
		return; //--------------------------------------------------------------
	}
...
void TootherWindow::onAdapterConnectableToggled() noexcept
{
	if (m_bSettingWidgetsValues) {
		return;
	}
...
void TootherWindow::onAdapterDetectableToggled() noexcept
{
	if (m_bSettingWidgetsValues) {
		return;
	}
...
void TootherWindow::onServiceRunningToggled() noexcept
{
	if (m_bSettingWidgetsValues) {
		return;
	}
...
void TootherWindow::onServiceEnabledToggled() noexcept
{
	if (m_bSettingWidgetsValues) {
		return;
	}
...
void TootherWindow::sendCmd(const std::string& sStr) noexcept
{
	int32_t nCount = 0;
...
void TootherWindow::printStringToLog(const std::string& sStr) noexcept
{
...
void TootherWindow::quitOnFatalError(const std::string& sErr) noexcept
{
	std::cerr << sErr << '\n';
