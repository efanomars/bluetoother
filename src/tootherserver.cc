...
TootherServer::TootherServer(int nCmdPipeFD, int nReturnPipeFD, bool bPrintOutServerErrors) noexcept
: m_nCmdPipeFD(nCmdPipeFD)
, m_nReturnPipeFD(nReturnPipeFD)
, m_bPrintOutServerErrors(bPrintOutServerErrors)
{
}
int TootherServer::run() noexcept
{
	m_refML = Glib::MainLoop::create();

	m_refPipeInputSource = Glib::RefPtr<PipeInputSource>(new PipeInputSource(m_nCmdPipeFD));
	m_refPipeInputSource->connect(sigc::mem_fun(*this, &TootherServer::doReceiveString));
	m_refPipeInputSource->attach();

	const std::string& sError = m_oHciSocket.init();
	if (! sError.empty()) {
		std::cerr << "Server error: " << sError << '\n';
		return EXIT_FAILURE;
	}
...
void TootherServer::doReceiveString(bool bError, const std::string& sStr) noexcept
{
...
void TootherServer::sendReturn(const std::string& sStr) noexcept
{
...
json TootherServer::getState() noexcept
{
...
void TootherServer::startTimeout(int32_t nStamp, int32_t nMillisec, bool bSendState) noexcept
{
	Glib::signal_timeout().connect_once(sigc::bind(sigc::mem_fun(*this, &TootherServer::onTimeout), nStamp, bSendState)
										, nMillisec);
}
void TootherServer::onTimeout(int32_t nStamp, bool bSendState) noexcept
{
...
void TootherServer::cmdGetState(int32_t nStamp) noexcept
{
...
void TootherServer::cmdSetAdapterSoftwareEnabled(int32_t nStamp, int32_t nAdapter, bool bEnabled) noexcept
{
...
void TootherServer::cmdSetAdapterIsUp(int32_t nStamp, int32_t nAdapter, bool bIsUp) noexcept
{
	cmdSetAdapterProperty(nStamp, nAdapter, 0, [&](HciAdapter* p0Adapter) {
		return p0Adapter->setAdapterIsUp(bIsUp);
	});
}
void TootherServer::cmdSetAdapterName(int32_t nStamp, int32_t nAdapter, const std::string& sName) noexcept
{
	cmdSetAdapterProperty(nStamp, nAdapter, 0, [&](HciAdapter* p0Adapter) {
		return p0Adapter->setLocalName(sName);
	});
}
void TootherServer::cmdSetAdapterConnectable(int32_t nStamp, int32_t nAdapter, bool bConnectable) noexcept
{
	cmdSetAdapterProperty(nStamp, nAdapter, 0, [&](HciAdapter* p0Adapter) {
		return p0Adapter->setConnectable(bConnectable);
	});
}
void TootherServer::cmdSetAdapterDetectable(int32_t nStamp, int32_t nAdapter, bool bDetectable) noexcept
{
	cmdSetAdapterProperty(nStamp, nAdapter, 0, [&](HciAdapter* p0Adapter) {
		return p0Adapter->setDetectable(bDetectable);
	});
}
void TootherServer::cmdServiceRunning(int32_t nStamp, bool bRun) noexcept
{
...
void TootherServer::cmdServiceEnabled(int32_t nStamp, bool bEnable) noexcept
{
