...
json JsonCommon::parseString(const std::string& sStr) noexcept
{
...
json JsonCommon::buildCmd(int32_t nStamp, const std::string& sCmd) noexcept
{
	assert(!sCmd.empty());
...
json JsonCommon::buildCmd(int32_t nStamp, const std::string& sCmd, int32_t nAdapter, bool bParam) noexcept
{
	assert(!sCmd.empty());
	json oCmdValue = json::object();
	oCmdValue[s_sKeyStamp] = nStamp;
...
json JsonCommon::buildCmd(int32_t nStamp, const std::string& sCmd, int32_t nAdapter, const std::string& sParam) noexcept
{
	assert(!sCmd.empty());
	json oCmdValue = json::object();
	oCmdValue[s_sKeyStamp] = nStamp;
...
json JsonCommon::buildReturnError(int32_t nStamp, const std::string& sError) noexcept
{
...
json JsonCommon::buildReturnOk(int32_t nStamp) noexcept
{
	json oReturnValue = json::object();
	oReturnValue[s_sKeyStamp] = nStamp;
...
json JsonCommon::buildReturnOk(int32_t nStamp, json&& oState) noexcept
{
	json oReturnValue = json::object();
	oReturnValue[s_sKeyStamp] = nStamp;
	oReturnValue[JsonCommon::s_sKeyReturnType] = s_sValueReturnTypeOk;
